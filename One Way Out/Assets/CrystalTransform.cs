using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class CrystalTransform : MonoBehaviour
{
    private GameManager gameManager;
    private SphereCollider trigger;

    private Vector3 startPos = new Vector3(-34.541f,2.232f,-1.426f);
    private Vector3 changedPos = new Vector3(-37.92f,1.719f,-3.608f);

    private bool canActivate = false;

    // Start is called before the first frame update
    void Start()
    {
        gameManager = GameObject.FindGameObjectWithTag("GameController").GetComponent<GameManager>();
        trigger = this.GetComponent<SphereCollider>();
        gameObject.transform.localPosition = startPos;
    }

    // Update is called once per frame
    void Update()
    {
        //The trigger is disabled when the boogie is activated. Once it's over the trigger gets enabled again.
        if (!gameManager.canActivateBoogie) return;
        else if (!trigger.enabled)
        {
            trigger.enabled = true;
        }

        if (canActivate && Input.GetKeyDown(KeyCode.E))
        {
            gameManager.ToggleCrystals();
            trigger.enabled = false;
            gameManager.SetInteractText(false);
            canActivate = false;

            if (!gameManager.crystalTransformed)
            {
                gameObject.transform.localPosition = startPos;
            }
            else
            {
                gameObject.transform.localPosition = changedPos;
            }

            trigger.enabled = true;
        }
    }

    private void OnTriggerEnter(Collider other)
    {
        canActivate = true;
        gameManager.SetInteractText(true);
    }
    
    private void OnTriggerExit(Collider other)
    {
        canActivate = false;
        gameManager.SetInteractText(false);
    }
}
